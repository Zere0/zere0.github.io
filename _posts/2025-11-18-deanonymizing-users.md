---
title: "When Blocking Becomes an Oracle: Deanonymizing Users at Scale"
...
In this post I will show how, by combining two perfectly "normal" features, it was possible to **recover the phone number of almost any user** on a large social media platform with millions of users.
...
## Where the Magic Happens: A Yes/No Oracle

When you follow someone on the platform, they appear in your `followings` list.

If you later **block the phone number** associated with that user, the blocked user **disappears** from your `followings` list.

This gives us a powerful **oracle**:

- **If a user disappears from `followings` after blocking a phone number → that phone number belongs to them.**
...
sequenceDiagram
    ...
    API-->>A: Victim appears in /followings

    A->>API: POST /api/v2/block (phone X)
    API-->>A: OK

    A->>API: GET /api/v2/followings
    API-->>A: [ ... ? ... ]

    Note over A,API: If victim disappears from /followings<br/>⬇<br/>X is their phone number
```
...
So, instead of blocking phone numbers one by one, we can block **large groups** and observe whether the victim disappears from the `followings` list. 

The high-level attack scenario is:

1. **Baseline**
  - Follow the victim so they appear in `/api/v2/followings`.
...
sequenceDiagram
    ...
    Note over A,API: Step 3 – Check followings
    A->>API: GET /api/v2/followings
    API-->>A: [ john, ralph ]
...
- Block the first 250 numbers and check `/api/v2/followings` again.
...
After at most log_2(500) ≈ 9 iterations, we are left with a **single phone number**.
...
- The script was run over the entire Andorra phone range, issuing one request per second: first a block request containing 500 phone numbers, followed by a request to read the `followings` list. In total, 1,200 requests would be sent, plus some additional block/unblock/check requests during the binary-search phase.
